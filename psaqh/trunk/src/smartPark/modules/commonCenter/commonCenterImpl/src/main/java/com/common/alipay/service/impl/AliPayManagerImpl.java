...
import com.common.alipay.common.AlipayCore;
...
public class AliPayManagerImpl extends BaseManagerImpl implements AliPayManager{
    ...
    /**
     * 返回支付宝请求参数link（app调用接口）
     */
    @EsbServiceMapping
    @Override
    public String getAlipayParamStr(@ServiceParam(name="userorderCode") String userorderCode)  throws BusException{
    	OrdermanagerUserorder order = ordermanagerUserorderManager.getOrderByCode(userorderCode);
        //商户订单号，商户网站订单系统中唯一订单号，必填
        String out_trade_no = order.getUserorderCode();
        String subject = order.getUserorderCode();
    ...
    @EsbServiceMapping
    @Override
    public String getAlipayapiHtml(@ServiceParam(name="userorderCode") String userorderCode)  throws BusException{
    	OrdermanagerUserorder order = ordermanagerUserorderManager.getOrderByCode(userorderCode);
        //商户订单号，商户网站订单系统中唯一订单号，必填
        String out_trade_no = order.getUserorderCode();
        String subject = order.getUserorderCode();
