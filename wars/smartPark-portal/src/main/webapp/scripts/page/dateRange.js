...
*/
    ...
    // 把时间选择框放到页面中
    $(0 < $('#appendParent').length ? '#appendParent' : $("#commit")).before(wrapper[this.mOpts.theme].join(''));
...
pickerDateRange.prototype.selectDate = function(ymd) {
    ...
    // start <-> end 切换123
		...
		//this.removeCSS(1);
        // 为当前点加样式
        $('#'+ this.calendarId + '_'  + ymd).toggleClass(this.mOpts.clickCss);//.attr('class',this.mOpts.clickCss);
		// 获取开始时间的初始值
        ...
        if (true == this.mOpts.singleCompare || true == this.mOpts.isSingleDay) {
			...
			(this.mOpts.shortOpr || this.mOpts.autoSubmit);
...
pickerDateRange.prototype.fillDate = function(year, month, index) {
	...
	if(isTaTheme){
		...
		if(this.mOpts.magicSelect){
			...
			$(cap).append("<span class='f20 pr30'>"+year+"</span>" + (month + 1) + '月');
		}
