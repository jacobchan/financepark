...
public class FileImportController {
	...
	//		List<Record> records = buildRecords(input, properties);
				...
				for(Record record:records){
					...
					for(Map.Entry<String, String> entry:properties.entrySet()){
						if("enTypeId".equals(entry.getKey())){//所在行业
							String enTypeName = record.get("enTypeId").toString();//实体
		...
		return new DataModelAndView(new Message("000000", "导入成功成功") );
	}
...
//						}
					...
					String name = savedMemeber.getMemberName();
					String phone = savedMemeber.getMemberPhoneNumber();
					String companyName = savedMemeber.getCompanyId();
					EnterbusinessmanagerRz ent = enterbusinessmanagerRzManager.getEnterbusinessmanagerRzByUniqueProperty("rzName", companyName);
					employe.setEmployeesName(name);
